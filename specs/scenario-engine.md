...
- **Scenario** — a named, curated base world-state (e.g. `standard-day`, `age-gated-order`, `multi-stop-route`). A scenario is a git tree: the world's sheets populated with fixture records. Scenarios live on a fixtures branch, one subtree per scenario, plus a shared `base/` subtree that all scenarios underlay.
...
```
refs/heads/fixtures                    # scenarios + base, human-curated, reviewed via PRs
...
1. **Fork (login)** — resolve the scenario named in the login request; build the session's initial tree by underlaying the shared `base/` tree beneath the scenario's subtree (scenario records win on conflict); commit it as the session's init commit with the fixtures commit as parent (provenance: `git log` traces every session back to the exact fixture state it forked from); point `refs/sessions/<key>` at it (create-only CAS — a concurrent login with the same key must fail loudly, not race).
...
- A session's history is a deterministic function of (fixtures commit, ordered request/event log). Replaying the same requests against the same fork must reproduce byte-identical trees — this is what makes sessions diffable across code versions and usable for agent evaluation.
